...
Database: a Supabase (Postgres) project — put `SUPABASE_DB_URL` in `.env` and load
with `python load_to_db.py` (see README). Schema in `schema_postgres.sql`; seed CSVs
in the project folder (or `seed_csv/` if you create one). `schema.sql` is the old
MySQL version, kept for reference only.
...
### Weather & cyclone risk (`weather.py` — not a DB table, computed on the fly)
Bay of Bengal cyclone-season climatology + per-port exposure, expressed as a
0-1 daily/window risk score. Feeds `is_monsoon`'s sibling feature
`cyclone_season_weight` into Person 2/3's models, an expected weather-risk
$ cost per (cargo, port) laycan window into Person 4's optimizer, a hard
block on ports whose risk is above `CYCLONE_HARD_BLOCK_THRESHOLD` for that
window, and a per-scenario severity multiplier into Person 5's Monte Carlo.
See the REAL vs SYNTHETIC table at the top of `weather.py` — short version:
the **shape** of the cyclone season (bimodal, pre-monsoon Apr-May / bigger
post-monsoon Oct-Dec, sharpest in Nov) and **which stretch of coast** takes
the most landfalls (Odisha/north AP) are real IMD/RSMC New Delhi
climatology; the exact numeric risk scores, delay-day estimates, and
demurrage rate used to price them in dollars are synthetic, calibrated to
that real pattern — same convention as the freight/commodity series above.
...
optimize(cargo_list, vessel_list, constraints, weather_multiplier=1.0) -> DataFrame[cargo, vessel, port, qty, cost_breakdown]
#   cost_breakdown keys: procurement_cost, freight_cost, vessel_fixed_cost, port_cost, weather_risk_cost, total_cost

# Person 5 - person5_risk_simulation.py
simulate_scenarios(n_scenarios: int)                                   -> DataFrame[scenario_id, optimized_cost, baseline_cost, savings_pct, weather_multiplier]

# weather.py (cross-cutting)
window_risk(port, start, end) / expected_delay_days(...) / weather_cost_adder(...) / is_extreme_risk(...)
```
...
GET  /weather/risk?port=Paradip&laycan_start=2026-10-20&laycan_end=2026-11-05
GET  /weather/climatology
GET  /health    GET /meta
```
...
afterwards with `python load_to_db.py`. The current generator
