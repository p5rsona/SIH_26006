...
Tries the database first (MySQL or Supabase — see db.py), then the CSV seed
...
def _try_load_from_db(route: str) -> pd.DataFrame | None:
    """Loads the index series from the database (MySQL or Supabase/Postgres —
    see db.py). Returns None if there's no database or no rows."""
